...
/**
 * ─────────────────────────────────────────────────────────────
 *  3D RENDER LOOKUP TABLE
 *  Built from the exact files in:
 *    /home/jpuser/projects/janapriyaupscale/backend/media/series/Nilevalley/Block-6/3D/
 *  Pattern: {floor_2digit}-{Facing}-{area_sqft}-SFT.png
 *  Served at: /media/series/Nilevalley/Block-6/3D/<filename>
 * ─────────────────────────────────────────────────────────────
 */
const BASE_3D = "/media/series/Nilevalley/Block-6/3D";

/** Every file that actually exists on disk, keyed by floor number. */
const FLOOR_3D_MAP: Record<number, Array<{ facing: "East" | "West"; area: number }>> = {
  ...
  // floor 24 not in folder — falls back to floor 25
  ...
  // floor 7 not in folder — falls back to floor 8
...
/** Nearest floor that has entries (handles floors 7, 24, etc.) */
...
/**
 * Build candidate 3D image URLs for a unit, ordered by best match:
 *   1. Exact floor + matching facing + closest area
 *   2. Exact floor + other facing + closest area
 *   3. Nearest floor (for gaps like 7 / 24) + same logic
 * The <Unit3DImage> component will try each URL in order until one loads.
 */
function get3DPaths(u: ApiUnit): string[] {
  const floor = u.floor_number;
  if (floor === undefined || floor === null) return [];

  const areaRaw = parseFloat((u.area_sqft || "0").toString().replace(/,/g, ""));
  const areaInt = isNaN(areaRaw) ? 0 : Math.round(areaRaw);

  // Resolve facing from unit fields
  const rawFacing = (u.facing || u.view || "").trim().toLowerCase();
  let primaryFacing: "East" | "West" | null = null;
  if (rawFacing.startsWith("e") || rawFacing.startsWith("n")) primaryFacing = "East";
  else if (rawFacing.startsWith("w") || rawFacing.startsWith("s")) primaryFacing = "West";

  // Helper: given a floor's entries, pick the best URL ordered by facing + closest area
  function urlsForFloor(floorNum: number): string[] {
    ...
    // Sort: preferred facing first, then by closest area
    const sorted = [...entries].sort((a, b) => {
      const aFacingMatch = primaryFacing ? (a.facing === primaryFacing ? 0 : 1) : 0;
      const bFacingMatch = primaryFacing ? (b.facing === primaryFacing ? 0 : 1) : 0;
      if (aFacingMatch !== bFacingMatch) return aFacingMatch - bFacingMatch;
      // Same facing priority → closest area wins
      return Math.abs(a.area - areaInt) - Math.abs(b.area - areaInt);
    });

    return sorted.map(e => `${BASE_3D}/${pad}-${e.facing}-${e.area}-SFT.png`);
  }

  const exactFloor  = urlsForFloor(floor);
  const closestFloor = nearestFloor(floor);
  const fallback    = closestFloor !== floor ? urlsForFloor(closestFloor) : [];

  // Deduplicate while preserving order
  return [...new Set([...exactFloor, ...fallback])];
}

/** Convenience: first candidate URL (used where a single string is needed) */
function get3DImage(u: ApiUnit): string | null {
  return get3DPaths(u)[0] || null;
}
...
@media(max-width:640px){
  ...
  /* hide desktop breadcrumb on mobile — show mobile row 2 instead */
  .pd-nav-row1 .pd-bc{display:none !important}
  .pd-nav-row2{
  ...
  .pd-bc a{font-size:12px}
  .pd-bc-cur{font-size:12px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:160px}
  .pd-nav-cta{padding:7px 14px;font-size:10px;white-space:nowrap;flex-shrink:0}
}
/* hide mobile row 2 on desktop */
@media(min-width:641px){
  .pd-nav-row2{display:none !important}
}
...
/* ── UNIT CARD — text left / 3D image right ── */
.pd-uc{
  background:#f5ead8;
...
.pd-uc:hover{background:#fff;box-shadow:0 4px 18px rgba(196,154,60,0.18)}
.pd-uc.sel{background:#fff;box-shadow:0 0 0 2px var(--gold)}
.pd-uc.unavail{opacity:0.38;cursor:not-allowed}
.pd-uc.hold{background:rgba(42,95,165,0.06)}
...
/* ── RIGHT: image — transparent bg, fixed 200×150 ── */
.pd-uc-img{
  flex:0 0 200px;
  width:200px;
  height:150px;
...
.pd-uc:hover .pd-uc-img img{transform:scale(1.05)}
.pd-uc-img-overlay{display:none}
...
.pd-uc-id{
  ...
  font-size:12px;color:#1a1208;
  margin-bottom:2px;
  white-space:nowrap;overflow:hidden;text-overflow:ellipsis;
  line-height:1.3;
}
.pd-uc-type{
  font-size:11px;color:#18437e;
  margin-bottom:4px;font-weight:600;
  letter-spacing:0.03em;
}
.pd-uc-price{
  font-size:11px;color:#1a1208;
...
.b-avail{background:rgba(196,154,60,0.18);color:#7a5a10;border:1px solid rgba(196,154,60,0.35)}
.b-hold{background:var(--sapphire-dim);color:var(--sapphire-lt);border:1px solid rgba(74,132,212,0.3)}
.b-sold{background:rgba(138,125,96,0.12);color:var(--muted);border:1px solid rgba(138,125,96,0.2)}

/* ── RESPONSIVE ── */
@media(max-width:1200px){
  .pd-unit-grid{grid-template-columns:repeat(3,minmax(0,1fr));gap:10px}
}
@media(max-width:900px){
  .pd-unit-grid{grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}
  .pd-uc-img,.pd-uc-img img,.pd-uc-img-placeholder{width:160px;height:120px;flex:0 0 160px}
}
@media(max-width:640px){
  .pd-unit-grid{grid-template-columns:1fr;gap:8px}
  .pd-uc{flex-direction:column-reverse;align-items:flex-start;padding:0 0 10px 0;min-height:unset}
  .pd-uc-text{flex:unset;width:100%;padding:0 12px}
  .pd-uc-img,.pd-uc-img img,.pd-uc-img-placeholder{width:100%;height:140px;flex:unset}
  .pd-uc-img img{width:100%;object-fit:contain}
}
...
.pd-pag-info strong{color:var(--gold)}
.pd-pag-controls{display:flex;align-items:center;gap:6px}
.pd-pag-btn{
  width:34px;height:34px;border-radius:3px;border:1px solid var(--border-gold);
  background:transparent;color:var(--muted);cursor:pointer;
  font-family:'Jost',sans-serif;font-size:12px;font-weight:500;
  display:flex;align-items:center;justify-content:center;
  transition:all 0.15s;
}
.pd-pag-btn:hover:not(:disabled){background:var(--gold-dim);color:var(--gold-lt);border-color:var(--gold)}
.pd-pag-btn:disabled{opacity:0.3;cursor:not-allowed}
.pd-pag-btn.active{background:var(--gold);color:var(--ink);border-color:var(--gold);font-weight:600}
.pd-pag-arrow{width:34px;height:34px;border-radius:3px;border:1px solid var(--border-gold);background:transparent;color:var(--muted);cursor:pointer;display:flex;align-items:center;justify-content:center;transition:all 0.15s;font-size:14px}
.pd-pag-arrow:hover:not(:disabled){background:var(--gold-dim);color:var(--gold-lt);border-color:var(--gold)}
.pd-pag-arrow:disabled{opacity:0.3;cursor:not-allowed}
.pd-pag-dots{font-size:12px;color:var(--muted);padding:0 2px}
@media(max-width:640px){
  .pd-pagination{justify-content:center}
  .pd-pag-info{width:100%;text-align:center}
}
...
.pd-footer-brand{display:flex;flex-direction:column;gap:8px}
.pd-footer-logo{font-family:'Playfair Display',serif;font-size:15px;color:var(--gold-lt);text-decoration:none;display:inline-block}
.pd-footer-logo em{font-style:italic;color:var(--gold)}
.pd-footer-tagline{font-size:10px;color:var(--muted);letter-spacing:0.08em;line-height:1.6;max-width:220px}
.pd-footer-links{display:flex;gap:2.5rem}
.pd-footer-col{display:flex;flex-direction:column;gap:6px}
.pd-footer-col-title{font-size:9px;letter-spacing:0.18em;text-transform:uppercase;color:var(--gold);margin-bottom:4px}
.pd-footer-col a,.pd-footer-col span{font-size:11px;color:var(--cream-2);text-decoration:none;opacity:0.7;transition:opacity 0.2s;cursor:default}
.pd-footer-col a{cursor:pointer}
.pd-footer-col a:hover{opacity:1;color:var(--gold-lt)}
.pd-footer-bottom{display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px}
.pd-footer-copy{font-size:10px;color:rgba(138,125,96,0.45);letter-spacing:0.04em}
.pd-footer-rera{font-size:10px;color:rgba(196,154,60,0.4);letter-spacing:0.06em}

/* Footer mobile */
@media(max-width:640px){
  .pd-footer{padding:1.6rem 1.2rem 1rem}
  .pd-footer-inner{flex-direction:column;gap:1.4rem}
  .pd-footer-links{flex-direction:column;gap:1.2rem}
  .pd-footer-bottom{flex-direction:column;align-items:flex-start;gap:4px}
  .pd-footer-tagline{max-width:100%}
}
...
@media(max-width:640px){
  ...
  .pd-unit-grid{grid-template-columns:repeat(2,minmax(0,1fr))}
...
/* ═══════════════════════════════════════════════════════════
   UNIT 3D IMAGE — auto-tries East → West, falls back to SVG
═══════════════════════════════════════════════════════════ */
function Unit3DImage({ unit, height = 150 }: { unit: ApiUnit; height?: number }) {
  ...
  // SVG fallback — transparent bg, matches card cream
  if (!paths.length || allFailed) {
    return (
      <div className="pd-uc-img-placeholder">
        <svg width="100" height="70" viewBox="0 0 100 70" style={{ opacity: 0.55 }}>
          ...
          <text x="25" y="28" textAnchor="middle" fontSize="6" fill="#C49A3C" fontFamily="Jost,sans-serif">BED</text>
          <text x="75" y="28" textAnchor="middle" fontSize="6" fill="#4A84D4" fontFamily="Jost,sans-serif">BED</text>
          <text x="25" y="48" textAnchor="middle" fontSize="6" fill="#C49A3C" fontFamily="Jost,sans-serif">LIVING</text>
          <text x="75" y="48" textAnchor="middle" fontSize="6" fill="#4A84D4" fontFamily="Jost,sans-serif">LIVING</text>
        </svg>
      </div>
    );
  }
  ...
  return (
    <img
      key={paths[tryIdx]}
      src={paths[tryIdx]}
      alt={`${unit.unit_number || unit.id} 3D`}
      onError={onError}
      style={{
...
function UnitSelector({ units, towers, slug, projectName, onExpressInterest, projectImages }: {
  ...
  // 3 rows × 3 cols (desktop=3cols per the grid). We use 15 = 3×5 desktop, but
  // simpler to derive from a fixed constant and let CSS handle visual rows.
  const COLS  = 5;   // cards per row (matches grid-template-columns:repeat(5,…))
  const ROWS  = 2;   // rows to show per page
  const PAGE_SIZE = COLS * ROWS; // 10

  /* ── Image resolution for detail panel only ── */
  ...
  // helper to build smart page number list e.g. [0,1,2,'…',8]
      ...
      <div className="pd-type-tabs">
        {types.map(t => (
          <button key={t} className={`pd-type-tab${activeType === t ? " on" : ""}`}
            onClick={() => { setActiveType(t); setSelIdx(0); setPage(0); }}>
            {t}
          </button>
        ))}
      </div>

      {/* Unit grid — paginated: 3 rows at a time */}
      {filtered.length > 0 ? (
        <>
          <div className="pd-unit-grid">
            {paginated.map((u, i) => {
              // global index in filtered list (for selIdx tracking)
              const globalIdx = page * PAGE_SIZE + i;
              return (
                <button key={u.id} className={ucCls(u, globalIdx)} onClick={() => { if (canClick(u)) setSelIdx(globalIdx); }}>

                  {/* ── LEFT: unit text ── */}
                  <div className="pd-uc-text">
                  ...
                  {/* ── RIGHT: 3D render image ── */}
                  <Unit3DImage unit={u} height={150} />
          ...
          {/* Pagination controls — shown only when more than one page */}
          {totalPages > 1 && (
            <div className="pd-pagination">
              ...
              <div className="pd-pag-controls">
                {/* Prev */}
                <button className="pd-pag-arrow" disabled={page === 0}
                  onClick={() => { setPage(p => p - 1); setSelIdx(0); }}
                  aria-label="Previous page">‹</button>

                {/* Page numbers */}
                {pageNums(page, totalPages).map((n, i) =>
                  n === "…"
                    ? <span key={`d${i}`} className="pd-pag-dots">…</span>
                    : <button key={n} className={`pd-pag-btn${page === n ? " active" : ""}`}
                        onClick={() => { setPage(n as number); setSelIdx(0); }}>
                        {(n as number) + 1}
                      </button>
                )}

                {/* Next */}
                <button className="pd-pag-arrow" disabled={page === totalPages - 1}
                  onClick={() => { setPage(p => p + 1); setSelIdx(0); }}
  ...
  return (
    <div className="pd">
      ...
      <nav className="pd-nav">
        {/* ── Desktop row (single flex) / Mobile row 1 ── */}
        <div className="pd-nav-row1" style={{ display: "flex", alignItems: "center", justifyContent: "space-between", width: "100%" }}>
          <Link href="/" className="pd-nav-logo">
            <img src="/logo-dark.png" alt="Janapriya Upscale" style={{ height: 40, width: "auto" }} />
          </Link>
          {/* Breadcrumb — hidden on mobile, shown on desktop */}
          <div className="pd-bc" style={{ display: "flex" }}>
            <Link href="/">Home</Link>
        ...
        {/* ── Mobile row 2: breadcrumb below logo ── */}
        <div className="pd-nav-row2">
          <div className="pd-bc">
            <Link href="/">Home</Link>
            <span className="pd-bc-sep">›</span>
            <Link href="/projects">Projects</Link>
      ...
      <div className="pd-cs">
        ...
        <div className="pd-ci-list">
          <div className="pd-ci">
            ...
            <div className="pd-ci-val">{project.address || "Site Office, Hyderabad"}</div>
          </div>
          <div className="pd-ci">
      ...
      <footer className="pd-footer">
        <div className="pd-footer-inner">
          ...
          <div className="pd-footer-brand">
            <Link href="/" className="pd-footer-logo">
              <img src="/logo-dark.png" alt="Janapriya Upscale" style={{ height: 36, width: "auto" }} />
            </Link>
            <div className="pd-footer-tagline">
              Crafting premium residences across Hyderabad.<br />
              {project.configuration || "Luxury 2 & 3 BHK"} in {project.city || "Hyderabad"}.
            </div>
          </div>
          ...
          <div className="pd-footer-links">
            ...
            <div className="pd-footer-col">
              <div className="pd-footer-col-title">{project.name}</div>
              ...
              {project.location && <span>{project.location}{project.city ? `, ${project.city}` : ""}</span>}
              {project.configuration && <span>{project.configuration}</span>}
            </div>

            {/* Quick links */}
            <div className="pd-footer-col">
              <div className="pd-footer-col-title">Explore</div>
              <Link href="/projects">All Projects</Link>
              <Link href="/units">Browse Units</Link>
              <a href="#enquire" onClick={e => { e.preventDefault(); scrollToEnquire(); }}>Enquire Now</a>
              {project.brochure_url && (
                <a href={mUrl(project.brochure_url)} target="_blank" rel="noopener noreferrer">Download Brochure</a>
              )}
            </div>

            {/* Contact — uses project.address from API, fallbacks are generic */}
            <div className="pd-footer-col">
              <div className="pd-footer-col-title">Contact</div>
              <span>{project.address || "Site Office, Hyderabad"}</span>
              <a href="tel:+914045678900">+91 40 4567 8900</a>
        ...
        <div className="pd-footer-bottom">
          <div className="pd-footer-copy">
            © 2026 Janapriya Engineers Syndicate Pvt. Ltd. · Hyderabad
          </div>
          {project.rera_number && (
            <div className="pd-footer-rera">RERA {project.rera_number}</div>
          )}
        </div>
      </footer>
