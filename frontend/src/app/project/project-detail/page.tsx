...
function Hero() {
  ...
  const total = 5;
  useEffect(() => {
    const t = setInterval(() => setDotIdx(i => (i + 1) % total), 3500);
  ...
  return (
    <div className="el-hero">
      ...
      {/* Dot indicators */}
      <div style={{ position:"absolute", bottom:84, left:"50%", transform:"translateX(-50%)", display:"flex", gap:6, zIndex:20 }}>
        {Array.from({ length: total }).map((_, i) => (
          <div key={i} onClick={() => setDotIdx(i)} style={{
            width: i === dotIdx ? 22 : 8, height:8, borderRadius:4,
            background: i === dotIdx ? "#f0c84a" : "rgba(255,255,255,0.5)",
