    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button onClick={rescoreAll} className="bg-gray-800 border border-gray-700 text-gray-300 rounded-lg px-3 py-2 text-sm hover:bg-gray-700">Rescore All</button>
          ...
          <tbody>
            ...
            {leads.map(lead => (
              <>
                ...
                {/* Expanded Detail Row */}
                {expandedId === lead.id && (
                  <tr key={`${lead.id}-detail`} className="bg-gray-800/40">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-xs">
                        ...
                        {lead.message && (
                          <div className="col-span-2 md:col-span-4">
                            ...
                            <p className="text-gray-300 bg-gray-900 rounded-lg px-3 py-2">{lead.message}</p>
                          </div>
                        )}
                        {lead.interest && (
                          <div>
                            <p className="text-gray-500 font-bold uppercase tracking-wider mb-1">Interest</p>
                            <p className="text-gray-300">{lead.interest}</p>
                          </div>
                        )}
                        ...
                        {lead.assigned_to && (
                          <div>
                            ...
                            <p className="text-gray-300">{lead.assigned_to}</p>
                          </div>
                        )}
                        {lead.notes && (
                        ...
                        {(lead.utm_source || lead.utm_medium || lead.utm_campaign) && (
                          <div className="col-span-2 md:col-span-4 border-t border-gray-700 pt-3 mt-1">
                          ...
                          <div className="col-span-2 md:col-span-4 border-t border-gray-700 pt-3 mt-1">
                            ...
                            <div className="flex flex-wrap gap-2">
                              {Object.entries(lead.score_details).filter(([k]) => k !== 'total').map(([k, v]) => (
                                <span key={k} className={`px-2 py-1 rounded text-xs ${Number(v) > 0 ? 'bg-green-900/30 text-green-300' : 'bg-gray-800 text-gray-500'}`}>
                        ...
                        {lead.sf_lead_id && (
                          <div>
                            ...
                            <p className="text-gray-300 font-mono">{lead.sf_lead_id}</p>
                          </div>
                        )}
                      </div>
                    </td>
...
function ScoreBadge({ score }: { score: number }) {
  let bg = 'bg-gray-800'; let text = 'text-gray-400';
